...
namespace crashhandler {
	...
	void CrashHandlerPrivate::initCrashHandlerPrivate(const std::string& report_minidumps_dirpath) {
	...
	pHandler = new google_breakpad::ExceptionHandler(
		...
		dumpCallback,		/* DumpCallback function, launched after writing minidump to file 
								should it return true => exception is treated as handled
								else => Breakpad treats exception as unhandled and allows another handler to handle it
							*/
	...
	{
		pPrivCrashHandler = new CrashHandlerPrivate();
	}

	CrashHandler::~CrashHandler()
	{
		delete pPrivCrashHandler;
	}
